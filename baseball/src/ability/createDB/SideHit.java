...
public class SideHit extends ConnectDB implements Data{
	...
	public SideHit(String username) {
		super("sidehit");
	...
	public void showLineChartData() {
		...
		while(true) {
			System.out.printf("1:側拋擊球率? 2:側拋擊遠 -1:結束", null);
			int options = input.nextInt();
			 if(options==-1)
				 break;
			if(options==1) {
				LineChart_AWT chart = new LineChart_AWT("Side Hit", "Side Hit %" ,dateArray,percentArray,"Percent of Side Hit");
			    chart.pack( );          
				...
				LineChart_AWT chart = new LineChart_AWT("Distance of Side Hit ", "Side Hit(m)" ,dateArray,distanceArray,"Distance of Side Hit");
			    chart.pack( );          
			    RefineryUtilities.centerFrameOnScreen( chart );          
			    chart.setVisible( true );
			    System.out.printf("-1:結束圖表", null);
	...
	*/
	...
	public JPanel showLineChartData(int options) {
		...
		if(options==1) {
			...
			LineChart t = new LineChart("Side Hit", "Side Hit %" ,dateArray,percentArray,"Percent of Side Hit");
			panel = t.getJPanel();
		}
		else if(options==2) {
			LineChart t = new LineChart("Distance of Side Hit ", "Side Hit(m)" ,dateArray,distanceArray,"Distance of Side Hit");
			panel = t.getJPanel();
		}
		return panel;
	}
